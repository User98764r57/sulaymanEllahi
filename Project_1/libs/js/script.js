...
const Jawg_Matrix = L.tileLayer('https://tile.jawg.io/jawg-matrix/{z}/{x}/{y}{r}.png?access-token=DSkywDmZAwXD3dsZbUNfIFHJvAUqdSVtMDe5eAbdJYhZUXJcg72lCMDNuqpf91UT', {
    attribution: 'Map data Â© Jawg Maps',
    noWrap: false,
    tileSize: 256,
    updateWhenIdle: true,
    keepBuffer: 8,
            ...
            if (data.geonames && data.geonames.length > 0) {
                data.geonames.forEach(city => {
                    ...
                    const marker = L.marker([lat, lng], { icon: cityIcon })
                        .bindPopup(`<b>${cityName}</b><br>Population: ${city.population}`);
...
overlayMaps["Capital Cities"] = cityClusterGroup;
    ...
    function fetchWeatherData(lat, lng) {
        $.ajax({
            ...
            success: function (data) {
                ...
                const weatherIcon = weatherEmojis[data.weather] || 'â›…ï¸';
    
                const weatherInfo = `
                    <div><strong>Temperature:</strong> ${data.temperature_c || 'N/A'}Â°C / ${data.temperature_f || 'N/A'}Â°F</div>
                ...
                forecastData.forEach((forecast, index) => {
                    ...
                    const weatherEmoji = weatherEmojis[forecast.weather] || 'â›…ï¸';
    
                    forecastInfo += `
                        <div><strong>${dayOfWeek}, ${forecast.date}</strong> ${weatherEmoji}</div>
                        <div>Temperature: ${forecast.temperature_c}Â°C / ${forecast.temperature_f}Â°F</div>
